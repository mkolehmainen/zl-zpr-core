...
NODE0_AUTH_PRIVATE_KEY="${NODE0_AUTH_PRIVATE_KEY:-$PREGEN/node0-rsa-key.pem}"
NODE1_AUTH_PRIVATE_KEY="${NODE1_AUTH_PRIVATE_KEY:-$PREGEN/node1-rsa-key.pem}"
...
NODE1_SUBSTRATE_ADDR_B=10.1.1.1
NODE1_SUBSTRATE_ADDR_C=10.1.2.1
NODE1_SUBSTRATE_ADDR_C_ALT=10.1.2.129  # Used for testing routing when a dock has multiple addresses.
NODE0_SUBSTRATE_ADDR_NODE=10.2.0.1
NODE1_SUBSTRATE_ADDR_NODE=10.2.0.2
VS_SUBSTRATE_ADDR=10.0.0.2
A_SUBSTRATE_ADDR=10.0.1.2
B_SUBSTRATE_ADDR=10.1.1.2
C_SUBSTRATE_ADDR=10.1.2.2
...
NODE0_SOCK=node0.sock
NODE1_SOCK=node1.sock
VS_SOCK=vs.sock
ADAPTER1_SOCK=adapter1.sock
ADAPTER2_SOCK=adapter2.sock
ADAPTER3_SOCK=adapter3.sock
NODE0_CAP_SOCK=node0_cap.sock
NODE1_CAP_SOCK=node1_cap.sock
...
cp "$PREGEN/node0.key" node0.key
cp "$PREGEN/node0-cert.pem" node0.crt
cp "$PREGEN/node0-pubkey.pem" node0.pubkey
cp "$PREGEN/node1.key" node1.key
cp "$PREGEN/node1-cert.pem" node1.crt
cp "$PREGEN/node1-pubkey.pem" node1.pubkey
...
sudo -E ip netns exec zpr-node0 sudo -E -u "$ZPR_USER" "$PH_BIN" \
  node \
  --logging "$DEBUG_TARGETS" \
  --control-path "$NODE0_SOCK" \
  --capture-path "$NODE0_CAP_SOCK" \
  --ca-file ca.crt \
  --certificate-file node0.crt \
  --private-key-file node0.key \
  --auth-private-key "$NODE0_AUTH_PRIVATE_KEY" \
  --km-impl "$KM_IMPL" \
  --tun-if tun0 \
  --zpr-addr "$NODE0_ZPR_ADDR" 2>&1 | tee node0.log | prefix_log zpr-node0 &

echo "Launching Node 1"

sudo -E ip netns exec zpr-node1 sudo -E -u "$ZPR_USER" "$PH_BIN" \
  node \
  --logging "$DEBUG_TARGETS" \
  --control-path "$NODE1_SOCK" \
  --capture-path "$NODE1_CAP_SOCK" \
  --ca-file ca.crt \
  --certificate-file node1.crt \
  --private-key-file node1.key \
  --auth-private-key "$NODE1_AUTH_PRIVATE_KEY" \
  --km-impl "$KM_IMPL" \
  --tun-if tun0 \
  --zpr-addr "$NODE1_ZPR_ADDR" 2>&1 | tee node1.log | prefix_log zpr-node1 &
...
wait_for 15 check_carrier zpr-node0 tun0 || { PASS=1; }
if [[ "$PASS" == 0 ]] then
wait_for 15 check_carrier zpr-node1 tun0 || { PASS=1; }
fi
if [[ "$PASS" == 0 ]] then
...
for SOCK in "$NODE0_SOCK" "$NODE1_SOCK" "$VS_SOCK" "$ADAPTER1_SOCK" "$ADAPTER2_SOCK"
do
